...
p3 <- cell_train %>% 
  ...
  scale_color_manual(values = c("normal" = "#A0A0A0", 
                                "diseased" = "red")) +
  labs(title = "Most Frequent Random Predictions") +
  ylab("Predicted Class") +
  theme(legend.position = "top", 
        axis.title = element_text(size = 7),
        axis.text = element_text(size = 7),
...
confusion_matrix <- data.frame(
  ...
  `Predicted Positive (PP)` = 
    c("True Positive (TP), Hit", "False Positive (FP), False Alarm, Overestimation"),
  `Predicted Negative (PN)` = 
    c("False Negative (FN), Miss, Underestimation", "True Negative (TN), Correct Rejection")
)
...
performance_metrics <- data.frame(
  ...
  Description = c("Overall correctness of the model", 
                  ...
                  "Correctly predicted positive observations to all observations in actual class", 
